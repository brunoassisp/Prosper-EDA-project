...
In this project, I am going to present an **Exploratory Data Analysis** over a dataset that contains informations about **Loans**. This dataset belongs to [Prosper](https://www.prosper.com/) and it contains *113,937 loans* with *81 variables* on each loan.
...
```{r}
head(subset(ld, !is.na(EstimatedEffectiveYield) & !is.na(EstimatedLoss) & !is.na(ProsperScore))) %>% skim()
```
